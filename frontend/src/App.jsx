...
function App() {
  ...
  async function removeEmployee(employee) {
    ...
    try {
      ...
      const result = await response.json()
      if (!response.ok) throw new Error(result.error || result.message || 'Unable to remove employee')
  ...
  function removeSelectedEmployee() {
    ...
    if (employee) removeEmployee(employee)
  }
  ...
  return (
    <div className="app-shell">
      ...
      {activeView === 'Employees' && <div className="employee-remove-bar"><span className="after-status-label">After status</span><select value={selectedEmployeeId} onChange={(event) => setSelectedEmployeeId(event.target.value)}><option value="">Select employee</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.first_name} {employee.last_name}</option>)}</select><button className="remove-button" onClick={removeSelectedEmployee} disabled={!selectedEmployeeId || deletingEmployeeId}>Remove</button></div>}
